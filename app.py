...
# Added command catalog; existing code/UI is preserved.
COMMAND_CATALOG = {
    "group_management": ["/groupname newname","/grouplock","/groupunlock","/removemember @user","/memberinfo @user","/groupinfo","/membercount","/listmembers","/adminlist","/setdesc description"],
    "auto_reply": ["/autoreplyon","/autoreplyoff","/setreply message","/getreply","/replytoall on/off","/delayreplied seconds","/replyemoji emoji","/replydisable @user","/replyenable @user","/clearreplyhist"],
    "youtube": ["/yt search_query","/ytplay video_name","/ytrandom","/yttrending","/ytstop","/ytdownload url","/ytinfo url","/ytsubtitle url","/ytplaylist url","/ytsearch latest"],
    ...
    "user_info": ["/whoami","/profile @user","/followercount @user","/followingcount @user","/postcount @user","/bio @user","/userjoined @user","/lastactive @user","/isverified @user","/getavatar @user"],
    "fun_commands": ["/joke","/meme","/roast @user","/compliment @user","/rate @user","/roll","/flip","/random","/trivia","/riddle"],
    "emoji_commands": ["/react emoji","/love","/laugh","/fire","/wow","/sad","/angry","/thumbsup","/thumbsdown","/celebrate"],
    "admin_commands": ["/setadmin @user","/removeadmin @user","/kick @user","/ban @user","/unban @user","/muteuser @user","/unmuteuser @user","/clearmessages @user","/lockgroup","/unlockgroup"],
    "bot_info": ["/help","/ping","/uptime","/version","/stats","/serverinfo","/botowner","/feedback message","/report issue","/donate"],
    "text_conversion": ["/uppercase text","/lowercase text","/reverse text","/bold text","/italic text","/strike text","/morse text","/binary text","/hex text","/ascii text"],
    "time_date": ["/time","/date","/time timezone","/countdown days","/daysleft event","/clock timezone","/timer seconds","/reminder message","/calendar","/upcoming"],
...
def full_command_help():
    ...
    for category, commands in COMMAND_CATALOG.items():
        ...
        out.extend("• " + c for c in commands)
    return "\n".join(out)
